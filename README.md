...
| 主题 | 文件 |
|------|------|
| **备考主线地图** — 培训课程表→已有笔记→熟练度状态、三阶段计划、实操盲操速查、理论重点、待确认清单 | [CCP备考-计划与考纲映射.md](./CCP备考-计划与考纲映射.md) |
| **集训预习速览** — 网络/HTTP/OS/数据库/前后端基础（渗透视角串联 + 预习自查清单） | [2026-09-07-CCP预习-Web渗透前置基础.md](./2026-09-07-CCP预习-Web渗透前置基础.md) |
| **考前复习总纲（闭卷版）** — 从 9/14-9/15 云帆文件管理系统靶场反推三道实操题的三项能力；熟练度自评（🟢已实操 / 🟡半熟 / 🧪纯理论未实操）；**必背 Payload 记忆卡 11 类**（SQL/XSS/CSRF/SSRF/上传/包含/RCE/XXE/反序列化/JWT/逻辑）；**工具操作卡**（Burp/sqlmap/哥斯拉/蚁剑/hashcat）；**防护必答表**；待实操清单 P0-P3；理论速记一页纸 | [2026-09-16-CCP考前复习-Web漏洞实操与必背清单.md](./2026-09-16-CCP考前复习-Web漏洞实操与必背清单.md) |
...
| **考前一晚总纲（单选/多选考点 + 三道实操 SOP）** — 按 9/7-9/16 培训课程表切块的 17 个笔试考点模块（网安基础/环境搭建/网络基础/Linux/信息收集/Burp/SQL/XSS/CSRF/SSRF/上传/越权/包含/XXE/大模型/全流程/法规，含多选陷阱标注） + 三道实操按概率排的 SOP（上传 getshell / SQL 注入 / SSRF / 登录爆破 / 解析漏洞 / SSTI，各带成功判据） + 通用开场 6 步五信号定栈 + 卡点速查表（现象→含义→下一步） + 明早进场前 30 分钟速览页 | [2026-09-18-CCP考前一晚-单选多选考点与实操SOP.md](./2026-09-18-CCP考前一晚-单选多选考点与实操SOP.md) |
### 加解密与编码
| 主题 | 文件 |
|------|------|
...
| **2026-09-16** | **CCP 考前复习总纲（闭卷） — 从 9/14-9/15 云帆文件管理系统靶场(Spring Boot 硬编码凭据+未授权 JSP 上传→哥斯拉 getshell root)反推三道实操题三项能力：指纹与接口挖掘 / 上传 getshell / 工具连接与报告撰写 · 熟练度自评:🟢6 项已实操(注入·XSS·越权·认证·上传·靶机提权) vs 🟡4 项半熟(CSRF·SSRF·XXE·上传绕过) vs 🧪12 项纯理论(文件包含/RCE/文件下载列为 P0 最危险) · 闭卷必背 Payload 11 类记忆卡 + Burp/sqlmap/哥斯拉/蚁剑 操作卡 + 防护必答表 12 行 + 待实操清单 P0-P3 时间盒 + 理论速记一页纸(状态码/端口/OSI/OWASP Top10/法规红线/大模型安全)** |
...
| **2026-09-18** | **CCP 考前一晚冲刺（题型=单选+多选+三道实操） — 培训表 17 模块笔试考点速记（OWASP Top10 / OSI 与端口表 / VMware 三网络模式 / Linux 权限与 SUID / Burp 四模块与 Intruder 四模式 / 四类注入手法对应现象 / 三类 XSS / XSS-CSRF-SSRF 三兄弟对比 / 水平 vs 垂直越权 / 上传检测六层与绕过阶梯 / 包含四 payload / XXE 禁用实体 / OWASP LLM Top10 与提示注入 / 渗透流程与报告要素） + 三道实操 SOP 与卡点速查表 + 进场前 30 分钟附页** |
| **2026-09-18(续)** | **PortSwigger 文件上传模块实战（Lab 3 路径穿越 + Lab 4 后缀黑名单绕过） — 核心认知：能上传与能执行是两套独立系统 · 「不解析」先分岔再选钥匙（位置问题→`..%2f` 穿越 / 类型问题→`.htaccess` 改后缀映射 / 没引擎→换 XSS·SSTI）· 响应头判执行 · `.htaccess` 速查与哨兵技巧 · 本机实测「PHP 不看后缀」· 踩坑：先清洗后解码、`.php3` 能传不解析、`l33t` vs `133t` 形近字符** |
