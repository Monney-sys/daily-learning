...
### CCP 集训实操（2026-09 · 已结项）

| 主题 | 文件 |
|------|------|
| **集训预习速览** — 网络/HTTP/OS/数据库/前后端基础（渗透视角串联 + 预习自查清单） | [2026-09-07-CCP预习-Web渗透前置基础.md](./2026-09-07-CCP预习-Web渗透前置基础.md) |
| **实操带练（闭卷清单的配套）** — 文件包含 DVWA 四档全通（low 零过滤 / medium 双写 `....//` / high 用 `file://` 绕前缀白名单 / impossible 固定值映射） · 命令执行四档全通（high 用 `| |whoami` 双写）· upload-labs **20 关检测逻辑源码级对照** + 通用绕过十阶梯 · 串链：上传 + 包含 = RCE · 排错口诀（空白 / 500 / Base64 三种现象的含义） | [2026-09-16(续)-CCP实操带练-文件包含与RCE与上传绕过.md](./2026-09-16(续)-CCP实操带练-文件包含与RCE与上传绕过.md) |
### 加解密与编码
| 主题 | 文件 |
|------|------|
...
| **PortSwigger 文件上传实战（Lab 1-4 全通：零防护 / Content-Type 绕过 / 路径穿越 / 后缀黑名单）（决策型）** — 两层模型（能落盘 vs 能执行）· 「不解析」两条分岔（位置问题→穿越 `..%2f` / 类型问题→`.htaccess` 改映射 / 没引擎→XSS·SSTI）· 响应头执行判据（text/plain+Last-Modified=静态直出 / text/html+Set-Cookie=执行 / 304=缓存障眼法）· `.htaccess` 语法·哨兵技巧·7 类攻击指令·同类物（.user.ini 300s 缓存）· 实测 php CLI 不看后缀 · 踩坑：形近字符 l33t vs 133t | [2026-09-18(续)-PortSwigger文件上传模块实战.md](./2026-09-18(续)-PortSwigger文件上传模块实战.md) |
...
| 主题 | 文件 |
|------|------|
| **应急响应（引入·总框架）** — 导图四阶段：表现（网站/文件/系统/流量/第三方异常症状）→ 收集（win&linux&mac 八项：对外服务/开放端口/系统版本/网络环境/漏洞情况/软件平台/口令整理/有无防护）→ 攻击（WEB 漏洞/结合/流量 · 第三方 数据库/远程软件/服务平台 · 操作系统 提权/内网渗透/远程漏洞）→ 追查（据表现选法：日志/后门/流量/脚本软件/模拟渗透分析）+ 案例2 WEB 首页被篡改（日志时间基线：前=攻击分析/后=操作分析 · webshell 查杀定位首次写入）+ 案例3 WIN 服务器卡顿（PCHunter64 进程监控 / UserAssistView 执行列表 / AppCompatCacheParser 时间线） | [2026-09-02-应急响应-总体框架与溯源.md](./2026-09-02-应急响应-总体框架与溯源.md) |
| **应急响应①（第一课：WEB 入侵排查展开）** — 流程五阶段（保护/分析/复现/修复/建议）+ 目的（攻击时间/操作/后果/安全修复）+ 准备工作 4 项（收集服务器信息/部署分析平台/整理工具指纹库/异常第一时间触发思路）+ 必备知识点 3 项（WEB 攻击技术/日志启用存储/日志数据分类分析）+ 常见分析方法 4 种（指纹库搜索/日志时间分析/后门追查分析/漏洞检查分析）+ 排查场景（有明确信息：基于时间/操作/指纹/其他；无明确信息：WEB→中间件→第三方→OS→口令后门 五层地毯式）+ 案例地图（案例1 Windows+IIS+Sql·日志搜索 / 案例2 Linux+BT_Nginx+tp5·日志后门 开场；案例3 Linux+Javaweb+st2、360星图 后续课） | [2026-09-03-应急响应-WEB入侵排查展开.md](./2026-09-03-应急响应-WEB入侵排查展开.md) |
| **应急响应·操作系统篇（Win/Linux 主机应急）** — 常见日志类别及存储（Win 事件日志 / Linux /var/log）+ 常见危害五类（爆破/漏洞利用/流量攻击/木马控制/病毒感染）+ 分析八项（账户/端口/进程/网络/启动/服务/任务/文件）+ 案例1 攻击响应-暴力破解 RDP/SSH（Win LogFusion 事件 ID / Linux secure 五连 grep：次数·时间范围·IP·用户名字典·成功登录）+ 案例2 控制响应-后门木马（CrossC2/geacon 生成 Linux C2 后门 + netstat -ntulp 自查）+ 案例3 危害响应-勒索 WannaCry（lesuobingdu.360.cn / nomoreransom.org 恢复站）+ 案例4 自动化响应-Gscan 脚本 | [2026-09-03-应急响应-操作系统应急响应WinLinux.md](./2026-09-03-应急响应-操作系统应急响应WinLinux.md) |
| **应急响应（收官·数据库与溯源）** — 数据库日志应急（MySQL general_log 三连 / MSSQL 查看跟踪分析 / Oracle 未展开）+ 案例1 LogonTracer（登录日志→neo4j 图可视化）· 案例3 自查漏洞模拟渗透溯源（WindowsVulnScan·linux-exploit-suggester·LinEnum·searchsploit·SNetCracker 弱口令）· 案例4 ir-rescue 工具箱（win/nix 取证采集）+ 必须知识点（打造自己的工具箱）· 待做：知攻善防应急响应靶场 | [2026-09-04-应急响应-数据库应急与漏洞口令检索.md](./2026-09-04-应急响应-数据库应急与漏洞口令检索.md) |
### 工具速查

| 主题 | 文件 |
|------|------|
| **工具速查库** — 信息收集/爆破注入/提权/内网横向/应急响应 五阶段命令速查（命令从旧笔记抽取、原样照抄+踩坑备注；打靶/做题/应急时查命令的落点） | [工具速查/README.md](./工具速查/README.md) |
...
| **2026-09-02** | **应急响应（引入）：总体框架与溯源 — 表现(网站篡改/系统卡顿/流量外连等5类症状) → 收集(8项) → 攻击(WEB/第三方/OS) → 追查(日志/后门/流量/脚本/模拟渗透) · 案例2 WEB被篡改(时间基线法) · 案例3 WIN卡顿(PCHunter64/UserAssistView/AppCompatCacheParser)** |
